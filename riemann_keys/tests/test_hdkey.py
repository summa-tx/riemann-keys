    ...
    def test_checksum(self):
        ...
        def test_public_key_generation():
            ...
            for test_vector in self.trezor_vectors['english']:
                ...
                root = HDKey.from_root_seed(
                    root_seed=root_seed)
                self.assertEqual(root.public_key, test_vector['root']['public_key'])
        ...
        def test_private_key_generation():
            ...
            for test_vector in self.trezor_vectors['english']:
                ...
                root = HDKey.from_root_seed(
                    root_seed=root_seed)
                self.assertEqual(root.private_key, test_vector['root']['private_key'])
        ...
        def test_xpub_setting():
            ...
            for test_vector in self.trezor_vectors['english']:
                ...
                self.assertEqual(
                    node.extended_public_key, test_vector['root']['xpub'])
                self.assertEqual(
                    node.public_key, test_vector['root']['public_key'])
                self.assertEqual(
            ...
            for test_vector in self.test_vectors['english']:
                ...
                self.assertEqual(
                    node.chain_code, test_vector['root']['chaincode'])
                self.assertEqual(root.depth, test_vector['root']['depth'])

        def test_xpriv_generation():
            # Test Trezor vectors.
            for test_vector in self.trezor_vectors['english']:
                root_seed = HDKey.root_seed_from_mnemonic(
                    mnemonic=test_vector.mnemonic)
        ...
        def test_node_path_derivation():
            ...
            for test_vector in self.trezor_vectors['english']:
                ...
                self.assertEqual(
                    node.public_key, test_vector['derived_node']['public_key'])
                self.assertEqual(
        ...
        def test_node_child_derivation():
            pass

        def test_public_keys_when_derived():
            # Test Trezor vectors.
            for test_vector in self.trezor_vectors['english']:
                root_seed = HDKey.root_seed_from_mnemonic(
                    mnemonic=test_vector.mnemonic)
        ...
        def test_private_keys_when_derived():
            # Test Trezor vectors.
            for test_vector in self.trezor_vectors['english']:
                root_seed = HDKey.root_seed_from_mnemonic(
                    mnemonic=test_vector.mnemonic)
                ...
                node = root.derive_path(test_vector['derived_node']['path'])
                self.assertEqual(
                    node.public_key, test_vector['derived_node']['private_key'])
        ...
        def test_xpub_when_derived():
            ...
            for test_vector in self.trezor_vectors['english']:
                ...
                node = root.derive_path(test_vector['derived_node']['path'])
                self.assertEqual(
                    node.public_key, test_vector['derived_node']['xpub'])
        ...
        def test_xpriv_when_derived():
            ...
            for test_vector in self.trezor_vectors['english']:
                ...
                node = root.derive_path(test_vector['derived_node']['path'])
                self.assertEqual(
                    node.public_key, test_vector['derived_node']['xpriv'])
            ...
            for test_vector in self.test_vectors['english']:
                ...
                node = root.derive_path(test_vector['derived_node']['path'])
                self.assertEqual(
                    node.public_key, test_vector['derived_node']['xrpiv'])

        def test_fingerprint_when_derived():
            # Test Trezor vectors.
            for test_vector in self.trezor_vectors['english']:
                root_seed = HDKey.root_seed_from_mnemonic(
                    mnemonic=test_vector.mnemonic)
                ...
                node = root.derive_path(test_vector['derived_node']['path'])
                self.assertEqual(
                    node.public_key, test_vector['derived_node']['fingerprint'])
